globalThis.JS = {
  ...
  callsSuper: function (a) {
    return a.SUPER === undefined ? (a.SUPER = /\bcallSuper\b/.test(a.toString())) : a.SUPER;
  },
  mask: function (a) {
...
JS.extend(JS.Module.prototype, {
  initialize: function (a, b) {
    ...
    this.__fns__ = {};
    this.__dep__ = [];
    this.__res__ = b._1 || null;
    this.include(a || {});
  },
  define: function (a, b, c) {
    c = c || {};
    this.__fns__[a] = b;
    if (JS.Module._0 && c._0 && JS.isFn(b)) JS.Module._0(a, c._0);
    var d = this.__dep__.length;
    while (d--) this.__dep__[d].resolve();
  },
  ...
  include: function (a, b, c) {
    ...
    if (a.__inc__ && a.__fns__) {
      this.__inc__.push(a);
      a.__dep__.push(this);
      if (b._2) a.extended && a.extended(b._2);
      else a.included && a.included(i);
    } else {
      if (b._5) {
        for (h in a) {
          if (JS.ignore(h, a[h])) continue;
          this.define(h, a[h], { _0: i || b._2 || this });
        }
      } else {
        if (typeof d === "object") {
          e = [].concat(d);
          for (g = 0, j = e.length; g < j; g++) i.include(e[g], b);
        }
        if (typeof f === "object") {
          e = [].concat(f);
        ...
        b._5 = true;
        return i.include(a, b, c);
      }
    }
    c && this.resolve();
  },
  includes: function (a) {
    if (Object === a || this === a || this.__res__ === a.prototype) return true;
    var b = this.__inc__.length;
    while (b--) {
      if (this.__inc__[b].includes(a)) return true;
    }
    return false;
  },
  ancestors: function (a) {
    a = a || [];
    for (var b = 0, c = this.__inc__.length; b < c; b++) this.__inc__[b].ancestors(a);
    var d = (this.__res__ || {}).klass,
      f = d && this.__res__ === d.prototype ? d : this;
    if (JS.indexOf(a, f) === -1) a.push(f);
    return a;
  },
  lookup: function (a) {
    var b = this.ancestors(),
      c = [],
      d,
      f,
      e;
    for (d = 0, f = b.length; d < f; d++) {
      e = b[d].__mod__.__fns__[a];
      if (e) c.push(e);
    }
    return c;
  },
  make: function (a, b) {
    if (!JS.isFn(b) || !JS.callsSuper(b)) return b;
    var c = this;
    return function () {
      return c.chain(this, a, arguments);
    };
  },
  chain: JS.mask(function (c, d, f) {
    var e = this.lookup(d),
      g = e.length - 1,
      j = c.callSuper,
      h = JS.array(f),
    ...
    i = e.pop().apply(c, h);
    j ? (c.callSuper = j) : delete c.callSuper;
    return i;
  }),
  resolve: function (a) {
    var a = a || this,
      ...
      e;
    if (a === this) {
      c = this.__dep__.length;
      while (c--) this.__dep__[c].resolve();
    }
    if (!b) return;
    for (c = 0, d = this.__inc__.length; c < d; c++) this.__inc__[c].resolve(a);
    for (f in this.__fns__) {
      e = a.make(f, this.__fns__[f]);
      if (b[f] !== e) b[f] = e;
    }
  },
});
JS.ObjectMethods = new JS.Module({
  __eigen__: function () {
    ...
    var a = (this.__meta__ = new JS.Module({}, { _1: this }));
    a.include(this.klass.__mod__);
    return a;
  },
  extend: function (a, b) {
    return this.__eigen__().include(a, { _2: this }, b !== false);
  },
  isA: function (a) {
  ...
  method: function (a) {
    var b = this,
      c = (b.__mcache__ = b.__mcache__ || {});
    if ((c[a] || {}).fn === b[a]) return c[a].bd;
    return (c[a] = { fn: b[a], bd: JS.bind(b[a], b) }).bd;
  },
});
JS.Class = JS.makeFunction();
JS.extend((JS.Class.prototype = JS.makeBridge(JS.Module)), {
  initialize: function (a, b) {
    var c = JS.extend(JS.makeFunction(), this);
    c.klass = c.constructor = this.klass;
    if (!JS.isFn(a)) {
      b = a;
      a = Object;
    }
  ...
  inherit: function (a) {
    this.superclass = a;
    if (this.__eigen__) {
      this.__eigen__().include(a.__eigen__ ? a.__eigen__() : new JS.Module(a.prototype));
    ...
    if (a !== Object) this.include(a.__mod__ || new JS.Module(a.prototype, { _1: a.prototype }), null, false);
  },
  include: function (a, b, c) {
    if (!a) return;
    var d = this.__mod__,
      b = b || {};
    b._4 = this;
    return d.include(a, b, c !== false);
  },
  extend: function (a) {
    if (!this.callSuper) return;
    this.callSuper();
    var b = this.subclasses.length;
    while (b--) this.subclasses[b].extend();
  },
  define: function () {
    var a = this.__mod__;
    a.define.apply(a, arguments);
    a.resolve();
  },
...
JS.Module.extend({
  ...
  _0: function (a, b) {
    var c = this._3,
      d = c.length;
    while (d--) c[d][0].call(c[d][1] || null, a, b);
  },
});
JS.extend(JS, {
  Interface: new JS.Class({
    initialize: function (d) {
      this.test = function (a, b) {
        var c = d.length;
        while (c--) {
          if (!JS.isFn(a[d[c]])) return b ? d[c] : false;
        }
        return true;
      };
    },
