jQuery(document).ready(function () {
  function geo_decode() {
    ...
    if (email !== undefined) {
      var email_name = email.innerHTML.split(/ /)[0];
      var email_domain = email.innerHTML.split(/ /)[2];
      var email_string = [email_name, email_domain].join("@");
  ...
  jQuery("#permission-request").bind("click", function () {
    ...
    jQuery.post("/permission_requests", { datatable: jQuery(this).data("datatableid") });
  });
  ...
  var usersPath = "/users?";
  jQuery.get(usersPath.concat(Math.random()));

  //Uses tokens for authors
  //    jQuery('#citation_author_block_input').tokenInput('/authors.json', { crossDomain: false });
});

// Called from legacy helpers (e.g. link_to_function 'remove_fields(this)').
globalThis.remove_fields = function (link) {
  jQuery(link).prev("input[type=hidden]").val("1");
  jQuery(link).parent().parent(".inputs").first().hide();
};

globalThis.add_fields = function (link, association, content) {
