...
class UnitTest < ActiveSupport::TestCase
  context 'human_name function' do
    setup do
      @speed = FactoryBot.build :unit, name: 'Feetpersecond', label: 'ft/2'
    end
    ...
    context 'and a name with one or more Pers in it' do
      setup do
        @speed = FactoryBot.build :unit, name: 'FeetPerSecond'
        @acceleration = FactoryBot.build :unit, name: 'FeetPerSecondPerSecond'
      end
    ...
    context 'and a name with no Pers in it' do
      setup do
        @distance = FactoryBot.build :unit, name: 'Feet'
      end
