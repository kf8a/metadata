...
class CitationTest < ActiveSupport::TestCase
  ...
  context 'Some citations exist at different dates' do
    setup do
      @oldcitation = FactoryBot.create(:citation, title: 'Old Citation')
      @oldcitation.updated_at = Date.civil(2000, 1, 1)
      @oldcitation.save
      @newcitation = FactoryBot.create(:citation, title: 'New Citation')
...
%M KBS.#{@citation.id}\n"
  ...
  context 'formatting a submitted citation' do
    ...
    should 'return the correct short_author_string' do
      result = 'Kaufman, A. S.'
      assert_equal result, @citation.short_author_string
    end
...
%M KBS.#{@citation.id}\n"
  ...
  context 'a citation with a doi but no volume' do
    setup do
      @cite = FactoryBot.build :citation, :doi => 'my_doi_string'
    end
  ...
  context 'a citation with no pub year' do
    setup do
      auth  = FactoryBot.build :author, :sur_name=>'Babbit', :given_name=>'Bob'
      @cite = FactoryBot.build :citation, :title => 'A title', :authors => [auth]
    end
  ...
  context 'a citation object with an author' do
    setup do
      @auth = FactoryBot.build :author, :sur_name=>'Babbit', :given_name=>'Bob'
      @cite = FactoryBot.build :citation
      @cite.authors << @auth
    end
  ...
  context 'a citation object with many authors' do
    setup do
      @cite = FactoryBot.create(:citation)
      @cite.authors = []
      4.times do
        @cite.authors << FactoryBot.build(:author, :given_name=>'Jon', :sur_name => 'Jones')
      end
      @cite.save
    end
