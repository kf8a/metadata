...
class PersonTest < ActiveSupport::TestCase
  ...
  context 'a person' do
    setup do
      @person = FactoryBot.create(:person, street_address: '34 main st. ', organization: '')
    end
  ...
  context 'a person with some lter_roles' do
    setup do
      @first_role = FactoryBot.create(:lter_role, name: 'Creators')
      @second_role = FactoryBot.create(:lter_role, name: 'Owners')
      @person = FactoryBot.create(:person, lter_roles: [@first_role, @second_role])
    end

    context 'some of the roles are committee roles' do
      setup do
        @first_committee = FactoryBot.create(:lter_role, name: 'Committee Members')
        @second_committee = FactoryBot.create(:lter_role, name: 'Network Representatives')
  ...
  context 'an emeritus' do
    setup do
      @emeritus = FactoryBot.build(:person, lter_roles: [FactoryBot.build(:role)])
    end
  ...
  context 'a non emeritus person' do
    setup do
      @person = FactoryBot.build(:person,
                                  lter_roles: [FactoryBot.build(:role,
                                                                 name: 'something else')])
    end
    ...
    should 'return false for #only_emeritus? if one role is non emeritus' do
      @person.lter_roles << FactoryBot.build(:role)
      assert !@person.only_emeritus?
    end
  end
  ...
  context 'a persons name' do
    setup do
      @person = FactoryBot.create(:person, given_name: 'howard',
                                            sur_name: 'spencer')
      @friendly_person = FactoryBot.create(:person, given_name: 'howard',
                                                     sur_name: 'spencer',
                                                     friendly_name: 'bops')
      @long_name = FactoryBot.create(:person, given_name: 'Shankurnarayanan',
  ...
  context 'dealing with non US address' do
    setup do
      @implicit_us_address = FactoryBot.create(:person, street_address: '208 Main St.',
                                                         city: 'Anytown',
                                                         locale: 'CA',
                                                         postal_code: '55555')
      @explicit_us_address = FactoryBot.create(:person,  street_address: '208 Main St.',
                                                          city: 'Anytown',
                                                          locale:  'CA',
                                                          postal_code: '55555',
                                                          country: 'USA')
      @less_explicit_us_address = FactoryBot.create(:person,  street_address: '208 Main St.',
                                                               city: 'Anytown',
                                                               locale: 'CA',
                                                               postal_code: '55555',
                                                               country: 'US')
      @eu_address = FactoryBot.create(:person, street_address: 'Hanover Strasse 20',
                                                city: 'Hanover',
                                                postal_code: '242435',
                                                country: 'Germany')
      @empty_country_address = FactoryBot.create(:person, street_address: '208 Main St.',
                                                           city: 'Anytown',
                                                           locale: 'CA',
                                                           postal_code: '55555',
                                                           country: '')
  ...
  context 'incomplete addresses' do
    setup do
      full_address = { street_address: '208 Main St.',
                       ...
                       postal_code: '55555',
                       country: 'USA' }
      @complete = FactoryBot.create(:person, full_address)
      @no_city =  FactoryBot.create(:person, full_address.merge(city: nil))
      @city_blank = FactoryBot.create(:person, full_address.merge(city: ' '))
      @city_empty = FactoryBot.create(:person, full_address.merge(city: ''))
      @no_street_address = FactoryBot.create(:person, full_address.merge(street_address: nil))
      @street_address_blank = FactoryBot.create(:person, full_address.merge(street_address: ' '))
      @no_postal_code = FactoryBot.create(:person, full_address.merge(postal_code: nil))
      @postal_code_blank = FactoryBot.create(:person, full_address.merge(postal_code: ' '))
      @no_locale = FactoryBot.create(:person, full_address.merge(locale: nil))
      @locale_blank = FactoryBot.create(:person, full_address.merge(locale: ' '))
      @eu_address = FactoryBot.create(:person, street_address: 'Hanover Strasse 20',
                                                city: 'Hanover',
                                                postal_code: '242435',
                                                country: 'Germany')
