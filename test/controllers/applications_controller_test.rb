...
class ApplicationControllerTest < ActionController::TestCase
  class FooController < ApplicationController
    ...
    before_action :admin?, only: [:testadmin]
    helper PeopleHelper
    ...
    def testpagechoose
      ...
      @roles = RoleType.find_by(:name, 'lter')
                       .roles.all(order: :seniority, conditions: ['name not like ?', 'Emeritus%'])
  ...
  class FooControllerTest < ActionController::TestCase
    ...
    #      setup do
    #        signed_in_as_admin
    #        FactoryBot.create(:role_type, :name => 'lter') unless RoleType.find_by_name('lter')
    #      end
    #
